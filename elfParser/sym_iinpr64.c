...
** Last update Tue Feb 14 21:37:01 2017 brout_m
*/
...
{
  if (elf64_section(elf, symbol->st_shndx)->sh_type != SHN_ABS &&
...
char          compareN64(Elf64_Ehdr *elf, Elf64_Sym *symbol)
{
  ...
  section = elf64_section(elf, symbol->st_shndx);
    if (!strcmp(&elf64_strtable(elf)[section->sh_name], ".debug"))
  {
    return ('N');
  }
  return (0);
...
char        compareP64(Elf64_Ehdr *elf, Elf64_Sym *symbol)
{
  ...
  section = elf64_section(elf, symbol->st_shndx);
  if (!strcmp(&elf64_strtable(elf)[section->sh_name], ".note.GNU-stack") &&
